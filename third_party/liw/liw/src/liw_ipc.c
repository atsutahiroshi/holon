...
/* METHOD: liwSHMModeAlloc
 * [SYNOPSIS]
 * int liwSHMModeAlloc(liwSHM *shm, char *key, int size, int mode);
 * [DESCRIPTION]
 * 'liwSHMModeAlloc()' allocates shared memory indicated with 'key'
 * on 'mode'.
 * 'size' is a size to allocate.
 * 'shm' is a pointer to the shared memory management instance.
 * [RETURN VALUE]
 * The value returned is the ID which indicates 'shm->mp'.
 */
int liwSHMModeAlloc(liwSHM *shm, char *key, int size, int mode)
{
 ...
 * [DESCRIPTION]
 * 'liwSHMClear()' clears up the memory space indicated by 'shm'.
 */
void liwSHMClear(liwSHM *shm)
{
 ...
 * detach shared memory indicated with 'id'.
 * return the true value if succeed.
 */
bool liwSHMDetach(liwSHM *shm)
{
  if( shmdt( shm->mp ) < 0 ){
...
/* METHOD: liwSHMDestroy
 * bool liwSHMDestroy(liwSHM *shm);
 * destroy shared memory indicated with 'id'.
 * return the true value if succeed.
 */
bool liwSHMDestroy(liwSHM *shm)
{
