...
void rkglMaterial(zOpticalInfo *oi)
{
  ...
  color[3] = zOpticalInfoAlpha(oi); /* alpha value */
  /* ambient */
  zRGB2fv( zOpticalInfoAmb(oi), color );
  glMaterialfv( GL_FRONT, GL_AMBIENT, color );
  /* diffuse */
  zRGB2fv( zOpticalInfoDif(oi), color );
  glMaterialfv( GL_FRONT, GL_DIFFUSE, color );
  /* specular */
  zRGB2fv( zOpticalInfoSpc(oi), color );
  glMaterialfv( GL_FRONT, GL_SPECULAR, color );
  /* shininess */
  color[0] = zOpticalInfoSns(oi);
